...
- [コネクションとページ分け](#コネクションとページ分け)
  ...
  - [ニュースフィードストーリーの無限スクロール](#ニュースフィードストーリーの無限スクロール)
    ...
    - [ステップ3 - 低水準なニュースフィードをフラグメントにいれる](#ステップ3---低水準なニュースフィードをフラグメントにいれる)
...
`Comment`コンポーネント内で個々のコメントを表示するために必要なデータを取得するために、ここに`CommentFragment`を展開しています。
...
それは、前と同じ`StoryFragment`を展開できる`Story`ノードです。
...
`on Query`でフラグメントを定義することで、最上位にそれを直接展開できます。
